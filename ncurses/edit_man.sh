...
(*.[0-9]*)
	...
	if test ! -f /workspace/3Labs/ncurses-6.1/man_alias.sed ; then
cat >>/workspace/3Labs/ncurses-6.1/man_alias.sed <<-CF_EOF2
		s,@DATADIR@,$datadir,g
...
s,@TSET@,tset,g
CF_EOF2
		echo "...made /workspace/3Labs/ncurses-6.1/man_alias.sed"
	fi
	...
	aliases=`sed -f $top_srcdir/man/manlinks.sed $inalias |sed -f /workspace/3Labs/ncurses-6.1/man_alias.sed | sort -u; test $inalias = $nCurses && echo curses`
	cf_target=`grep "^$cf_source" /workspace/3Labs/ncurses-6.1/man/man_db.renames | mawk '{print $2}'`
	...
	sed	-f /workspace/3Labs/ncurses-6.1/man_alias.sed \
		< $i | sed -f /workspace/3Labs/ncurses-6.1/edit_man.sed >$TMP
